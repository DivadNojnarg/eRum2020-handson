...
5. Now, let's try to change the color of the `<p>` tag. Hint: use the following pattern `$(selector).css('color', ...)` where you need to replace the selector by the good element and `...` by the color of your choice. The `css` method may also be apply to change other properties.
6. Add it the `class = "awesome-text"` attribute. Hint: use the `$(selector).addClass(class)` pattern.
7. Add it the `id = "text1"` attribute. Hint: to add an attribute, `$(selector).attr(name, value)` does the job.
8. Note that all above steps may be chained as follows:

```{js}
$(selector)
  .css("color", "your_color")
  .addClass("...")
  .attr("", "");
```

The last step is always followed by `;`. 
...
3. Create an event listener to change the third item color as soon as one click on it.
