...
3. Create a code to bundle each of those dependencies, applying your recent {htmltools} knowldege.
...
In the following, we will introduce the `{jstools}` package developed by [dreamRs](https://github.com/dreamRs/jstools), which provides a R wrapper for various JavaScript tools like [JSHint](https://jshint.com/). You could also directly use JSHint.

6. The js folder of this project contains a custom input binding. This is this code which drives the tabler navigation menu. Run the following code. 

```{r}
bindings <- list.files(
  path = here("2_case_study/js"),
  recursive = TRUE,
  full.names = TRUE
)
jshint_file(input = bindings, options = jshint_options(esversion = 6, jquery = TRUE, globals = list("Shiny", "app")))
```
